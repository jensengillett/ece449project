...
begin
    process(clk) begin
        if(rising_edge(clk) and decode_enable = '1') then  -- Decode always occurs on rising edge.
            if(in_flush_pipeline = '1') then  -- if the pipeline needs to be flushed
                alu_op                 <= (others => '0');
                mem_op                 <= (others => '0');
                wb_op                  <= '0';
                read_1_select          <= (others => '0');
                read_2_select          <= (others => '0');
                write_select           <= (others => '0');
                cl_value               <= (others => '0');
                branch_op              <= (others => '0');
                branch_displacement    <= (others => '0');
                out_in_port            <= (others => '0');
                out_in_port_enable     <= '0';
                in_write_data          <= (others => '0');
                in_port_index          <= (others => '0');
                out_enable             <= '0';
                out_m1                 <= '0';
                out_imm                <= (others => '0');
        ...
        if(falling_edge(clk) and decode_enable = '1') then  -- Latch for input port. 
